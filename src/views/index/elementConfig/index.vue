    ...
    <div class="modify-style event">
        <div class="event-title">
            ...
            <div style="display: inline-block;" width="49%" @click="() => {
                ...
                lockEl = !lockEl }">
                <v-btn v-if="lockEl" color="black"> <LockOutlined />元素嵌入</v-btn>
                <v-btn v-else color="black"> <UnlockOutlined />元素游动</v-btn>
            </div>
        </div>
        <div class="modify-style-content">
            <div v-show="!!vnodeStore.curVnode && !!vnodeStore.curVnode.id">
                ...
                <div class="item" v-for="key, index in Object.keys(finalStyle)" :key="index" v-show="finalStyle[key].value !== ''">
                    <VCombobox :list="finalStyle[key].list" :ZHname="finalStyle[key].descriptions" :ENname="key" :modelValue="finalStyle[key].value"></VCombobox>
                </div>
            </div>
        </div>
    </div>
